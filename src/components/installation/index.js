...
const handleEdit = (props, item) => {
    props.history.push(`/installation/edit/${item}`)
}
...
const fields = ['area_name','area_id', 'status', 'action']
...
class Tables extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:8080/webapi/listinstallation').then(res=>{
  ...
  handleDelete = (id) => {
    if(window.confirm('Are you sure want to delete?')) {
      axios.delete(`http://localhost:8080/webapi/deleteinstallation/${id}`,
      {
        // headers: {
            ...
            <CCard>
              <CCardHeader>
                ...
                <div className="card-header-actions">
                  <span  rel="noreferrer noopener" className="card-header-action">
